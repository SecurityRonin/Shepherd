...
pub fn plugin() -> EcosystemPlugin {
    EcosystemPlugin {
        name: "docling",
        description: "Docling MCP server for PDF/DOCX/PPTX/XLSX document conversion",
        ...
        config_content: DOCLING_MCP_JSON,
    }
}

const DOCLING_MCP_ENTRY: &str = r#""docling": {
      "command": "uvx",
      "args": ["--from=docling-mcp", "docling-mcp-server"],
      "env": {}
    }"#;

const DOCLING_MCP_JSON: &str = "{\n  \"mcpServers\": {\n    \"docling\": {\n      \"command\": \"uvx\",\n      \"args\": [\"--from=docling-mcp\", \"docling-mcp-server\"],\n      \"env\": {}\n    }\n  }\n}";
...
pub fn is_docling_compatible(agent: &str) -> bool {
...
mod tests {
    ...
    fn test_detect_claude_code_user_scope() {
        ...
        std::fs::write(
            ...
            r#"{"mcpServers":{"docling":{"command":"uvx","args":["--from=docling-mcp","docling-mcp-server"]}}}"#,
        ).unwrap();
        let result = detect_for_agent("claude-code", tmp.path(), None);
        assert!(result.installed);
    ...
    fn test_install_config_generates_mcp_json() {
        ...
        assert!(config.mcp_server_json.contains("docling"));
        assert!(config.mcp_server_json.contains("docling-mcp-server"));
    ...
    fn test_supported_agents() {
        assert!(is_docling_compatible("claude-code"));
        assert!(is_docling_compatible("codex"));
        assert!(is_docling_compatible("gemini-cli"));
        assert!(!is_docling_compatible("aider"));
    }
