...
mod tests {
    ...
    fn test_detect_claude_code_plugin_dir() {
        let tmp = tempfile::tempdir().unwrap();
        let plugin_dir = tmp.path().join(".claude/plugins/cache/anthropics/claude-code/frontend-design");
    ...
    fn test_detect_claude_code_settings_ref() {
        ...
        std::fs::write(
            ...
            r#"{"plugins":["frontend-design@claude-code-plugins"]}"#,
        ).unwrap();
        let result = detect_for_agent("claude-code", tmp.path(), None);
        assert!(result.installed);
        assert_eq!(result.scope, InstallScope::User);
    ...
    fn test_supported_agents() {
        assert!(is_frontend_design_compatible("claude-code"));
        assert!(!is_frontend_design_compatible("codex"));
    }
