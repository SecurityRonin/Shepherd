...
pub fn plugin() -> EcosystemPlugin {
    EcosystemPlugin {
        name: "serena",
        description: "Serena MCP server for LSP-powered semantic code navigation and editing",
        ...
        config_content: SERENA_MCP_JSON,
    }
}

const SERENA_MCP_ENTRY: &str = r#""serena": {
      "command": "uvx",
      "args": ["--from", "git+https://github.com/oraios/serena", "serena", "start-mcp-server"],
      "env": {}
    }"#;

const SERENA_MCP_JSON: &str = "{\n  \"mcpServers\": {\n    \"serena\": {\n      \"command\": \"uvx\",\n      \"args\": [\"--from\", \"git+https://github.com/oraios/serena\", \"serena\", \"start-mcp-server\"],\n      \"env\": {}\n    }\n  }\n}";
...
pub fn is_serena_compatible(agent: &str) -> bool {
...
mod tests {
    ...
    fn test_detect_claude_code_user_scope() {
        ...
        std::fs::write(
            ...
            r#"{"mcpServers":{"serena":{"command":"uvx","args":["--from","git+https://github.com/oraios/serena","serena","start-mcp-server"]}}}"#,
        ).unwrap();
        let result = detect_for_agent("claude-code", tmp.path(), None);
        assert!(result.installed);
    ...
    fn test_install_config_generates_mcp_json() {
        ...
        assert!(config.mcp_server_json.contains("serena"));
        assert!(config.mcp_server_json.contains("start-mcp-server"));
        assert!(config.mcp_server_json.contains("oraios/serena"));
    ...
    fn test_supported_agents() {
        assert!(is_serena_compatible("claude-code"));
        assert!(is_serena_compatible("codex"));
        assert!(is_serena_compatible("gemini-cli"));
        assert!(!is_serena_compatible("aider"));
    }
