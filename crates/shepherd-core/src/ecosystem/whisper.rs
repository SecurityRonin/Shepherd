...
/// Return the shared plugin definition for local-stt-mcp (Whisper).
pub fn plugin() -> EcosystemPlugin {
    EcosystemPlugin {
        name: "local-stt",
        description: "Local speech-to-text MCP server using whisper.cpp with speaker diarization",
        compatible_agents: &["claude-code", "codex", "gemini-cli"],
        feature_key: "local-stt",
        plugin_cache_dirs: &[],
        user_settings_paths: &[
        ...
        config_content: LOCAL_STT_MCP_JSON,
    }
}

const LOCAL_STT_MCP_ENTRY: &str = r#""local-stt": {
      "command": "npx",
      "args": ["-y", "local-stt-mcp"],
      "env": {}
    }"#;

const LOCAL_STT_MCP_JSON: &str = "{\n  \"mcpServers\": {\n    \"local-stt\": {\n      \"command\": \"npx\",\n      \"args\": [\"-y\", \"local-stt-mcp\"],\n      \"env\": {}\n    }\n  }\n}";
...
pub fn is_whisper_compatible(agent: &str) -> bool {
...
mod tests {
    ...
    fn test_detect_claude_code_user_scope() {
        ...
        std::fs::write(
            ...
            r#"{"mcpServers":{"local-stt":{"command":"npx","args":["-y","local-stt-mcp"]}}}"#,
        )
        .unwrap();
        let result = detect_for_agent("claude-code", tmp.path(), None);
        assert!(result.installed);
    ...
    fn test_detect_claude_code_project_scope() {
        ...
        std::fs::write(
            ...
            r#"{"mcpServers":{"local-stt":{"command":"npx"}}}"#,
        )
        .unwrap();
    ...
    fn test_install_config_generates_mcp_json() {
        ...
        assert!(config.mcp_server_json.contains("local-stt"));
        assert!(config.mcp_server_json.contains("local-stt-mcp"));
    ...
    fn test_supported_agents() {
        assert!(is_whisper_compatible("claude-code"));
        assert!(is_whisper_compatible("codex"));
        assert!(is_whisper_compatible("gemini-cli"));
        assert!(!is_whisper_compatible("aider"));
    }
